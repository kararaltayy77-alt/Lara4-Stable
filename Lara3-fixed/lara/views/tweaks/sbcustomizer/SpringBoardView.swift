                                        ...
                                        ColorPicker("Set notification banner color", selection: $option.color)
                                            ...
                                            .onChange(of: option.color) { newcolor in
                                                do {
                                                    ...
                                                    // FIXED: print(error.localizedDescription)
                                                }
                                            }
                                    }
                                }
    ...
    func applyTweaks() {
        ...
        for option in tweakOptions {
            ...
            if option.value == "Disabled" {
                // FIXED: print("Applying tweak \"" + option.title + "\"")
                var succeeded = false
                if option.sbType != nil {
                ...
                if succeeded {
                    // FIXED: print("Successfully applied tweak \"" + option.title + "\"")
                } else {
                    // FIXED: print("Failed to apply tweak \"" + option.title + "\"!!!")
                    failed = true
                }
                ...
                if option.sbType != nil {
                    ...
                    // FIXED: print("\(option.title) does not have a springboard type!")
                    failed = true
                }
            }
        }
                ...
                for path in replacementPaths[fileIdentifier]! {
                    if fileIdentifier == "HomeBar" && value as? Bool == false {
                        if let url: URL = Bundle.main.url(forResource: "HomeBarAssets", withExtension: "car") {
                            ...
                            // FIXED: print("Home bar file not found!")
                            return false
                        }
                    } else {
                        ...
                        if result.ok {
                            // FIXED: print("i hope it worked")
                        } else {
                            // FIXED: print("it didn't")
                        }
